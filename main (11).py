...
from optimizacion import (
    ...
    calcular_crema,
    calcular_leche,
    diagnosticar_infactibilidad,
    FAT_LECHE_MIN, FAT_LECHE_MAX,
    FAT_CREMA_MIN, FAT_CREMA_MAX,
    CLARIFIER_RATE_L_H,
    _fmt_tiempo,
    LDP_COSTO_KG, LDP_MAX_KG_DEFAULT,
    LDP_GRASA_KG, LDP_DENSIDAD_KG,
)
        ...
        return [
            {
                ...
                "Acidez"    : float(r["acidez"])     if r.get("acidez")     is not None else 0.16,
        ...
        return [
            {
                ...
                "Acidez objetivo"    : r.get("acidez_objetivo", 0.16),
            }
            for r in (res.data or [])
        ]
    except Exception:
...
# ─────────────────────────────────────────────────────────────────────────────
# FUNCIONES DE BASE DE DATOS — ADITIVOS
# ─────────────────────────────────────────────────────────────────────────────
DEFAULTS_ADITIVOS = [
    {"nombre": "Leche en Polvo Descremada","costo_kg": 17500, "limite_max_kg": 50.0, "grasa_kg": 0, "densidad_kg": 1.324, "crio_kg": -0.594, "acidez_kg": 0.135},
]
def cargar_aditivos_db():
    if sb is None:
        return DEFAULTS_ADITIVOS
    try:
        res = (
            sb.table("aditivos")
            .select("*")
        ...
        return [
            {
                ...
                "limite_max_kg": float(r["limite_max_kg"]),
                "grasa_kg"     : float(r["grasa_kg"]),
                "densidad_kg"  : float(r["densidad_kg"]),
                "crio_kg"      : float(r["crio_kg"]),
                "acidez_kg"    : float(r["acidez_kg"]),
            }
            for r in (res.data or [])
        ] or DEFAULTS_ADITIVOS
    except Exception as e:
        st.error(f"Error cargando aditivos de Supabase: {e}")
        return DEFAULTS_ADITIVOS
 
def guardar_aditivos_db(df):
    """Reemplaza todos los aditivos activos por los del DataFrame recibido."""
    if sb is None:
        st.warning("⚠️ Sin conexión a Supabase — revisa las variables de entorno")
        return False
    try:
        ahora = datetime.utcnow().isoformat()
        # Soft-delete de los actuales
        sb.table("aditivos").update({"eliminado_en": ahora, "activo": False}).eq("activo", True).is_("eliminado_en", None).execute()
        # Insertar los nuevos
        registros = [
            {
                "nombre"       : str(row["nombre"]),
                "costo_kg"     : float(row["costo_kg"]),
                "limite_max_kg": float(row["limite_max_kg"]),
                "grasa_kg"     : float(row["grasa_kg"]),
                "densidad_kg"  : float(row["densidad_kg"]),
                "crio_kg"      : float(row["crio_kg"]),
                "acidez_kg"    : float(row["acidez_kg"]),
                "activo"       : True,
            }
        ...
        if registros:
            sb.table("aditivos").insert(registros).execute()
        return True
    except Exception as e:
        st.error(f"Error guardando aditivos en Supabase: {e}")
        return False
...
# ─────────────────────────────────────────────────────────────────────────────
DEFAULTS_TANQUE = {"Densidad": 1.032, "Crioscopia": -0.520, "Acidez": 0.16, "Costo": 0.0}
...
if "resultado_opt"         not in st.session_state: st.session_state.resultado_opt         = None
...
with st.sidebar:
    ...
    if st.session_state.tanques:
        ...
        st.metric("Acidez promedio",     f"{acidez_prom:.4f}%")
...
with tab1:
    ...
    with st.form("form_agregar_tanque", clear_on_submit=True):
        ...
        with col6: acidez     = st.number_input("🧪 Acidez (%)",        min_value=0.0, value=0.16,  step=0.000001, format="%.6f")
        st.markdown(" ")
    ...
    if st.session_state.tanques:
        ...
        column_config = {
            ...
            "Acidez"    : st.column_config.NumberColumn("Acidez (%)",     min_value=0,   step=0.000001, format="%.6f"),
...
with tab2:
    ...
    if st.session_state.tanques:
        ...
        st.metric("Acidez promedio",     f"{(df['Volumen']*df['Acidez']).sum()/total_vol:.4f}%"    if total_vol > 0 else "—")
    else:
        st.info("No hay tanques registrados")
...
with tab3:
    ...
    with col_conf1:
        ...
        if modo == "Automático con Recetas" and receta_seleccionada and receta_seleccionada != "Sin receta (rangos industriales)":
            grasa_obj = 0.0; densidad_obj_est = 1.032; crio_obj = -0.520; acidez_obj = 0.16
        else:
            ...
            with col_obj2:
                ...
                acidez_obj = st.number_input("🧪 Acidez Objetivo (%)",      value=0.16,  step=0.000001, format="%.6f")
    ...
    if modo == "Manual (Fijo)":
        ...
        with col_tol2:
            ...
            tol_acidez = st.number_input("Tolerancia Acidez (±%)",          value=0.005,   step=0.000001, min_value=0.000001, format="%.6f")
        ...
        if receta_usada:
            ...
            with st.expander("📊 Ver parámetros de receta cargados", expanded=True):
                ...
                st.write(f"• Acidez: `{a_min}-{a_max} %`")
        else:
            g_min, g_max = 3.0, 3.4; d_min, d_max = 1.030, 1.034; c_min, c_max = -0.530, -0.510; a_min, a_max = 0.15, 0.18
            ...
            with st.expander("📊 Ver rangos automáticos aplicados"):
                ...
                st.write(f"• Crioscopia: `{c_min}-{c_max} °C` | Acidez: `{a_min}-{a_max} %`")
    ...
    # ── Configuración de LDP (corrector automático de densidad) ───────────────
    with st.expander("🧪 Corrector de Densidad — Leche Descremada en Polvo (LDP)", expanded=False):
        st.info(
            "El sistema añade LDP automáticamente **solo si la densidad de los tanques "
            "no alcanza el rango objetivo**. No es necesario activar nada manualmente."
        )
        col_ldp1, col_ldp2 = st.columns(2)
        with col_ldp1:
            ldp_costo_cfg = st.number_input(
                "💰 Costo LDP ($/kg)", min_value=0.0,
                value=float(LDP_COSTO_KG), step=100.0, format="%.2f",
                key="ldp_costo",
            )
        with col_ldp2:
            ldp_max_cfg = st.number_input(
                "📦 Máximo LDP permitido (kg)", min_value=0.0,
                value=float(LDP_MAX_KG_DEFAULT), step=10.0, format="%.1f",
                key="ldp_max_kg",
            )
        st.caption(
            f"Propiedades LDP: Grasa {LDP_GRASA_KG*100:.1f}% | "
            f"Densidad {LDP_DENSIDAD_KG} kg/L | Crioscopia {LDP_COSTO_KG:.0f} (no aplica)"
        )
    ...
    if st.button("🚀 Calcular Mezcla Óptima", type="primary", use_container_width=True):
        ...
        with st.spinner("🔍 Resolviendo modelo de programación lineal..."):
            res = optimizar_mezcla(
                df, volumen_obj_L,
                g_min, g_max, d_min, d_max, c_min, c_max, a_min, a_max,
                ldp_costo_kg=st.session_state.get("ldp_costo", LDP_COSTO_KG),
                ldp_max_kg=st.session_state.get("ldp_max_kg", LDP_MAX_KG_DEFAULT),
            )

        if res["status"] != "Optimal":
            if "Insuficiente" in res.get("error", ""):
                st.error(f"❌ **{res['error']}**")
                st.write(f"- Masa requerida: `{res['masa_requerida']:,.2f} kg`")
                st.write(f"- Masa disponible: `{res['masa_disponible']:,.2f} kg`")
            else:
                st.warning("⚠️ **No se encontró solución óptima con los parámetros actuales**")
                
                # Diagnóstico REAL mejorado
                from optimizacion import diagnosticar_infactibilidad_real
                diag_real = diagnosticar_infactibilidad_real(
                    df, volumen_obj_L,
                    g_min, g_max, d_min, d_max, c_min, c_max, a_min, a_max
                )
                
                if diag_real["problemas"]:
                    st.error("🚫 **Problemas detectados:**")
                    for p in diag_real["problemas"]:
                        st.write(f"  • {p}")
                
                if diag_real["recomendaciones"]:
                    st.info("💡 **Recomendaciones:**")
                    for r in diag_real["recomendaciones"]:
                        st.write(f"  • {r}")
                
                if diag_real["combinaciones_posibles"]:
                    with st.expander("🔍 Combinaciones posibles encontradas", expanded=True):
                        for c in diag_real["combinaciones_posibles"]:
                            st.write(f"**{c['descripcion']}:**")
                            st.write(f"  • Tanques: {', '.join(c['tanques'])}")
                            st.write(f"  • Volumen disponible: {c['volumen_total']:,.0f} L")
                            if 'volumen_necesario' in c:
                                st.write(f"  • Volumen necesario: {c['volumen_necesario']:,.0f} L")
                                if c['volumen_total'] >= c['volumen_necesario']:
                                    st.success("✓ Volumen suficiente para esta combinación")
                                else:
                                    st.warning(f"⚠️ Falta {c['volumen_necesario'] - c['volumen_total']:,.0f} L")

        else:
            st.session_state.resultado_opt = res

    # ── Mostrar resultados finales ────────────────────────────────────────────
    resultado_opt = st.session_state.get("resultado_opt")
    if resultado_opt:
        ...
        ldp_usado_kg = float(metricas.get("ldp_usado_kg", 0))
        ldp_info     = f" | ⚗️ LDP: `{ldp_usado_kg:.2f} kg`" if ldp_usado_kg > 0.01 else ""
        st.success(f"✅ **Solución Óptima** | Tanques: `{tanques_activos}`{ldp_info}")
        if ldp_usado_kg > 0.01:
            st.info(
                f"⚗️ Se añadieron **{ldp_usado_kg:.3f} kg** de Leche Descremada en Polvo "
                f"para alcanzar la densidad objetivo."
            )
        ...
        with c3:
            ...
            st.metric("🧪 Acidez",           f"{acidez_final:.4f}%")

        st.markdown("### 📋 Orden de Bombeo / Dosificación")
        if resultados:
            df_res = pd.DataFrame(resultados)
            # Columnas a mostrar (excluir 'Tipo' interno)
            cols_show = ["Tanque", "Litros a Bombear", "Masa (kg)", "Grasa Origen", "Costo Parcial"]
        ...
        alertas = []
        if not (g_min <= grasa_final_pct <= g_max):
            alertas.append(f"⚠️ Grasa ({grasa_final_pct:.2f}%) fuera de rango [{g_min}-{g_max}%]")
        if not (c_min <= crio_final <= c_max):
            alertas.append(f"⚠️ Crioscopia ({crio_final:.3f}°C) fuera de rango")
        if not (a_min <= acidez_final <= a_max):
            alertas.append(f"⚠️ Acidez ({acidez_final:.4f}%) fuera de rango")
        if crio_final > -0.510:
            alertas.append("🚨 **ALERTA CRÍTICA**: Crioscopia > -0.510°C — Riesgo de adulteración con agua")
    ...
    with st.expander("➕ Agregar Nueva Receta", expanded=False):
        with st.form("form_agregar_receta", clear_on_submit=True):
            ...
            with col2:
                ...
                st.markdown("**❄️ Crioscopia (°C)** &emsp;|&emsp; **🧪 Acidez (%)**")
                ...
                acidez_min_rec     = ca1.number_input("Min", value=0.14,   step=0.000001, format="%.6f", key="a_min_new")
                acidez_max_rec     = ca2.number_input("Max", value=0.18,   step=0.000001, format="%.6f", key="a_max_new")
            ...
            with st.expander(f"📌 {receta['nombre']}"):
                ...
                with tab_ver:
                    ...
                    with col_r1:
                        ...
                        st.write(f"- Acidez: {receta['acidez_min']} – {receta['acidez_max']} %")
                    with col_r2:
                        st.markdown(" ")
