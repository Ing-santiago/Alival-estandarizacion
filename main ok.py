...
with tab3:
    ...
    with st.expander("🧂 Adición de Sólidos y Aditivos (Opcional)", expanded=False):
        ...
        if usar_aditivos:
            ...
            with col_save:
                if st.button("💾 Guardar en base de datos", key="btn_guardar_aditivos", use_container_width=True):
                    if _df_aditivos is not None and not _df_aditivos.empty:
                        if guardar_aditivos_db(_df_aditivos):
                            st.session_state.aditivos_config = cargar_aditivos_db()
                            st.success("✅ Aditivos guardados y disponibles para todos los usuarios.")
            with col_reset:
                if st.button("↩️ Restaurar valores originales", key="btn_reset_aditivos", use_container_width=True):
                    if guardar_aditivos_db(pd.DataFrame(DEFAULTS_ADITIVOS)):
                        st.session_state.aditivos_config = cargar_aditivos_db()
                        st.rerun()
            if _df_aditivos is not None and not _df_aditivos.empty:
    ...
    if st.button("🚀 Calcular Mezcla Óptima", type="primary", use_container_width=True):
        with st.spinner("🔍 Resolviendo modelo de programación lineal..."):
            ...
            # ── DETECCIÓN AUTOMÁTICA DE ALTA ACIDEZ ──────────────────────────────
        if resultado_opt["status"] != "Optimal":
            st.warning("⚠️ **No se encontró solución óptima con los parámetros actuales**")
        
            # Verificar si es problema de acidez
            if "Insuficiente" not in resultado_opt.get("error", ""):
                st.info("🤖 **Intentando ajuste automático con Sustrato S...**")
            
                from optimizacion import intentar_ajuste_con_sustrato_s
            
                with st.spinner("🧪 Calculando ajuste de acidez con Sustrato S..."):
                    resultado_con_ajuste = intentar_ajuste_con_sustrato_s(
                        df, volumen_obj_L,
                        g_min, g_max, d_min, d_max, c_min, c_max, a_min, a_max,
                        acidez_limite_critica=13.8,
                    )
            
                if resultado_con_ajuste["status"] == "Optimal" and resultado_con_ajuste.get("ajuste_sustrato_s"):
                    st.success("✅ **Solución encontrada con ajuste automático de Sustrato S**")
                
                    ajuste = resultado_con_ajuste["ajuste_sustrato_s"]
                
                    # Mostrar métricas del ajuste
                    st.markdown("### 🧪 Ajuste de Acidez Aplicado")
                    col_a1, col_a2, col_a3 = st.columns(3)
                    with col_a1:
                        st.metric("📊 Acidez Inicial", f"{ajuste['acidez_inicial']:.2f} °D")
                    with col_a2:
                        st.metric("📉 Reducción", f"{ajuste['reduccion_acidez']:.2f} °D", delta_color="normal")
                    with col_a3:
                        st.metric("🎯 Acidez Final", f"{ajuste['acidez_final']:.2f} °D", 
                                 delta=f"{ajuste['acidez_final'] - ajuste['acidez_inicial']:+.2f} °D")
                
                    col_b1, col_b2 = st.columns(2)
                    with col_b1:
                        st.metric("💧 Volumen Sustrato S", f"{ajuste['volumen_sustrato_s']:,.3f} L")
                    with col_b2:
                        st.metric("❄️ Crioscopia Final", f"{ajuste['crioscopia_final']:.4f} °C",
                                 delta=f"{ajuste['crioscopia_final'] - ajuste['crioscopia_inicial']:+.4f} °C")
                
                    # Mostrar advertencias
                    if ajuste.get("advertencias"):
                        with st.expander("⚠️ Advertencias del ajuste", expanded=True):
                            for adv in ajuste["advertencias"]:
                                st.warning(adv)
                
                    # Confirmación del usuario
                    st.markdown("---")
                    st.info("💡 El sistema ha calculado automáticamente el volumen de Sustrato S necesario para corregir la acidez.")
                
                    aceptar_ajuste = st.checkbox(
                        "✓ Acepto aplicar el ajuste con Sustrato S",
                        key="chk_aceptar_ajuste_sustrato",
                    )
                
                    if aceptar_ajuste:
                        resultado_opt = resultado_con_ajuste
                        st.success("✅ Ajuste aplicado. Continuando con la optimización...")
                        st.rerun()
                    else:
                        st.warning("⚠️ Sin el ajuste, la optimización no es viable.")
                        st.stop()
                else:
                    st.error(f"❌ **El ajuste con Sustrato S tampoco funcionó**: {resultado_con_ajuste.get('error', 'Error desconocido')}")
                    st.stop()
            else:
                # Error de materia prima insuficiente
        ...
        # ── VERIFICACIÓN POST-OPTIMIZACIÓN DE ALTA ACIDEZ ────────────────────
        # Incluso si la optimización fue exitosa, verificar si la acidez es muy alta
        elif resultado_opt["status"] == "Optimal":
            metricas_temp = resultado_opt["metricas"]
            acidez_final_temp = metricas_temp.get("acidez_final", 0)
        
            if acidez_final_temp > 13.8:
                st.warning(f"⚠️ **ALERTA: Acidez muy alta ({acidez_final_temp:.2f}°D) - Supera el límite de 13.8°D**")
                st.info("🤖 **Sugiriendo ajuste automático con Sustrato S...**")
            
                from optimizacion import intentar_ajuste_con_sustrato_s
            
                with st.spinner("🧪 Calculando ajuste de acidez..."):
                    resultado_con_ajuste = intentar_ajuste_con_sustrato_s(
                        df, volumen_obj_L,
                        g_min, g_max, d_min, d_max, c_min, c_max, a_min, a_max,
                        acidez_limite_critica=13.8,
                ...
                if resultado_con_ajuste["status"] == "Optimal" and resultado_con_ajuste.get("ajuste_sustrato_s"):
                    st.success("✅ **Ajuste con Sustrato S disponible**")
                
                    ajuste = resultado_con_ajuste["ajuste_sustrato_s"]
                
                    col_a1, col_a2, col_a3 = st.columns(3)
                    with col_a1:
                        st.metric("📊 Acidez Actual", f"{ajuste['acidez_inicial']:.2f} °D", delta="⚠️ Alta")
                    with col_a2:
                        st.metric("💧 Sustrato S Requerido", f"{ajuste['volumen_sustrato_s']:,.3f} L")
                    with col_a3:
                        st.metric("🎯 Acidez Corregida", f"{ajuste['acidez_final']:.2f} °D", delta="✅ Dentro de rango")
                
                    aceptar_ajuste_post = st.checkbox(
                        "✓ Aplicar ajuste con Sustrato S para corregir acidez",
                        key="chk_ajuste_post_opt",
                    )
                
                    if aceptar_ajuste_post:
                        resultado_opt = resultado_con_ajuste
                        st.info("✅ Ajuste aplicado. Revise los resultados actualizados abajo.")
                    else:
                        st.error("⚠️ La mezcla tiene acidez fuera de especificación. Se recomienda aplicar el ajuste.")

        # Si llegamos aquí, tenemos una solución válida
