...
try:
    ...
    st.error(f"❌ Error importando módulo de optimización: {_e}. Verifica que `pulp>=2.7.0` esté en requirements.txt")
    st.stop()
...
# Asistente IA WebSocket eliminado por requerimiento del sistema
...
with tab1:
    ...
    with st.form("form_agregar_tanque", clear_on_submit=True):
        ...
        if submitted:
            ...
            elif len(st.session_state.tanques) >= 10:
                # Límite de 10 tanques simultáneos
                st.error("⚠️ Límite máximo de 10 tanques alcanzado. Elimina alguno antes de agregar otro.")
            else:
                tanque_id = agregar_tanque_db(tanque, volumen, grasa, densidad, crioscopia, acidez, costo)
                if tanque_id:
                    # Recargar siempre desde DB tras el INSERT para garantizar
                    # que el estado local refleja exactamente lo que hay en Supabase
                    st.session_state.tanques = cargar_tanques_db()
                    st.success(f"✅ Tanque '{tanque}' agregado. Total: {len(st.session_state.tanques)} tanque(s)")
                    st.rerun()
                else:
                    st.error("❌ No se pudo guardar el tanque en la base de datos. Revisa la conexión a Supabase.")

    if st.session_state.tanques:
        st.markdown("---")
        n_tanques = len(st.session_state.tanques)
        st.subheader(f"📋 Inventario de Tanques  ({n_tanques}/10)")
        if n_tanques >= 10:
            st.warning("⚠️ Se alcanzó el límite de 10 tanques. Elimina alguno para poder agregar más.")
        elif n_tanques >= 8:
            st.info(f"ℹ️ {10 - n_tanques} espacio(s) disponible(s) de 10.")
            ...
            if st.button("🗑️ Eliminar", type="primary", use_container_width=True):
                ...
                if tanque_data:
                    if eliminar_tanque_db(tanque_data["id"]):
                        # Recargar desde DB para mantener sincronía con Supabase
                        st.session_state.tanques = cargar_tanques_db()
                        st.rerun()
