...
Ajuste automático con Sustrato S para corrección de acidez
"""
import pulp
import pandas as pd
import math
...
# ── Ajuste automático con Sustrato S ──────────────────────────────────────

def calcular_ajuste_sustrato_s(
    acidez_actual: float,
    acidez_objetivo_max: float,
    volumen_leche: float,
    crioscopia_inicial: float = -0.520,
) -> dict:
    """
    Calcula el volumen de Sustrato S necesario para reducir la acidez.
    
    Fórmulas:
    - Volumen Sustrato S = ((Acidez Actual - Acidez objetivo) / 16666 * Volumen) × 100
    - Alteración crioscopia = crioscopia_inicial + (-196 * (Volumen S / Volumen leche))
    - Acidez final = Acidez inicial + (-196 * (Volumen S / Volumen leche))
    
    Args:
        acidez_actual: Acidez actual de la mezcla (°D)
        acidez_objetivo_max: Acidez máxima objetivo (°D) - típicamente 13.8
        volumen_leche: Volumen total de leche (L)
        crioscopia_inicial: Crioscopia inicial de la mezcla (°C)
    
    Returns:
        dict con volumen_sustrato_s, acidez_final, crioscopia_final, y validaciones
    """
    if volumen_leche <= 0:
        return {
            "valido": False,
            "error": "El volumen de leche debe ser mayor a 0",
        }
    
    if acidez_actual <= acidez_objetivo_max:
        return {
            "valido": False,
            "error": f"La acidez actual ({acidez_actual:.2f}°D) ya está dentro del límite ({acidez_objetivo_max}°D)",
            "requiere_ajuste": False,
        }
    
    # Calcular volumen de Sustrato S
    # Fórmula: ((Acidez Actual - Acidez objetivo) / 16666 * Volumen) × 100
    volumen_sustrato_s = ((acidez_actual - acidez_objetivo_max) / 16666 * volumen_leche) * 100
    
    # Calcular factor de corrección
    # Fórmula: -196 * (Volumen de S / Volumen leche)
    if volumen_leche > 0:
        factor_correccion = -196 * (volumen_sustrato_s / volumen_leche)
    else:
        factor_correccion = 0
    
    # Calcular acidez final y crioscopia final
    acidez_final = acidez_actual + factor_correccion
    crioscopia_final = crioscopia_inicial + factor_correccion
    ...
    # Validar que la acidez final esté dentro del límite
    if acidez_final > acidez_objetivo_max:
        valido = False
        errores.append(f"La acidez final ({acidez_final:.2f}°D) aún excede el límite ({acidez_objetivo_max}°D)")
    
    if acidez_final < 0:
        valido = False
        errores.append(f"La acidez final calculada es negativa ({acidez_final:.2f}°D) - valor inválido")
    
    # Validar crioscopia
    if crioscopia_final > -0.510:
        advertencias.append(f"⚠️ Crioscopia final ({crioscopia_final:.4f}°C)接近 límite de adulteración (-0.510°C)")
    
    if crioscopia_final < -0.540:
        advertencias.append(f"⚠️ Crioscopia final ({crioscopia_final:.4f}°C) muy baja - verificar")
    
    # Validar volumen de sustrato
    if volumen_sustrato_s > volumen_leche * 0.15:
        advertencias.append(f"⚠️ El volumen de Sustrato S ({volumen_sustrato_s:.2f} L) representa más del 15% del volumen total")
    
    return {
        "valido": valido,
        "requiere_ajuste": True,
        "volumen_sustrato_s": round(volumen_sustrato_s, 3),
        "acidez_inicial": round(acidez_actual, 2),
        "acidez_final": round(acidez_final, 2),
        "acidez_objetivo": acidez_objetivo_max,
        "crioscopia_inicial": round(crioscopia_inicial, 4),
        "crioscopia_final": round(crioscopia_final, 4),
        "factor_correccion": round(factor_correccion, 4),
        "reduccion_acidez": round(acidez_actual - acidez_final, 2),
        "errores": errores,
        "advertencias": advertencias,
    }


def intentar_ajuste_con_sustrato_s(
    df,
    volumen_obj_L,
    g_min, g_max,
    d_min, d_max,
    c_min, c_max,
    a_min, a_max,
    acidez_limite_critica: float = 13.8,
) -> dict:
    """
    Intenta resolver una optimización infactible aplicando Sustrato S automáticamente.
    
    Estrategia:
    1. Primero optimiza sin considerar acidez (solo grasa, densidad, crioscopia)
    2. Calcula la acidez resultante
    3. Si excede el límite, calcula Sustrato S necesario
    4. Verifica si con el ajuste la solución es viable
    
    Args:
        df: DataFrame con tanques disponibles
        volumen_obj_L: Volumen objetivo (L)
        ...
        acidez_limite_critica: Límite crítico de acidez (default 13.8°D)
    
    Returns:
        dict con status, resultados, metricas, y ajuste_sustrato_s si aplica
    """
    densidad_estimada = (d_min + d_max) / 2
    masa_objetivo_Kg = volumen_obj_L * densidad_estimada
    
    # Paso 1: Optimizar SIN restricción de acidez (solo grasa, densidad, crioscopia)
    modelo_temp = pulp.LpProblem("Estandarizacion_Sin_Acidez", pulp.LpMinimize)
    
    x_kg_temp = {
        i: pulp.LpVariable(f"Kg_Tanque_{i}", lowBound=0, upBound=df.loc[i, "Masa_Disponible_Kg"])
        for i in df.index
    }
    
    # Función objetivo: minimizar costo
    costo_temp = pulp.lpSum([x_kg_temp[i] * df.loc[i, "Costo_Por_Kg"] for i in df.index])
    modelo_temp += costo_temp, "Costo_Minimo"
    
    # Restricciones sin acidez
    masa_temp = pulp.lpSum([x_kg_temp[i] for i in df.index])
    modelo_temp += masa_temp == masa_objetivo_Kg, "Meta_Masa"
    
    # Grasa
    grasa_temp = pulp.lpSum([x_kg_temp[i] * (df.loc[i, "Grasa"] / 100.0) for i in df.index])
    modelo_temp += grasa_temp >= masa_objetivo_Kg * (g_min / 100.0), "Min_Grasa"
    modelo_temp += grasa_temp <= masa_objetivo_Kg * (g_max / 100.0), "Max_Grasa"
    
    # Densidad
    densidad_temp = pulp.lpSum([x_kg_temp[i] * float(df.loc[i, "Densidad"]) for i in df.index])
    modelo_temp += densidad_temp >= masa_objetivo_Kg * d_min, "Min_Densidad"
    modelo_temp += densidad_temp <= masa_objetivo_Kg * d_max, "Max_Densidad"
    
    # Crioscopia
    crio_temp = pulp.lpSum([x_kg_temp[i] * float(df.loc[i, "Crioscopia"]) for i in df.index])
    modelo_temp += crio_temp >= masa_objetivo_Kg * c_min, "Min_Crioscopia"
    modelo_temp += crio_temp <= masa_objetivo_Kg * c_max, "Max_Crioscopia"
    
    # Resolver
    solver_temp = pulp.PULP_CBC_CMD(msg=False, timeLimit=30)
    modelo_temp.solve(solver_temp)
    
    if pulp.LpStatus[modelo_temp.status] != "Optimal":
        return {
            "status": "Infeasible",
            "error": "No se encontró solución ni siquiera sin considerar acidez",
            "ajuste_sustrato_s": None,
        }
    
    # Calcular métricas de la solución temporal
    masa_total_temp = sum(x_kg_temp[i].value() for i in df.index if x_kg_temp[i].value())
    grasa_total_temp = sum(x_kg_temp[i].value() * (df.loc[i, "Grasa"] / 100.0) for i in df.index if x_kg_temp[i].value())
    crio_total_temp = sum(x_kg_temp[i].value() * df.loc[i, "Crioscopia"] for i in df.index if x_kg_temp[i].value())
    acidez_total_temp = sum(x_kg_temp[i].value() * df.loc[i, "Acidez"] for i in df.index if x_kg_temp[i].value())
    
    acidez_resultante = acidez_total_temp / masa_total_temp if masa_total_temp > 0 else 0
    crioscopia_resultante = crio_total_temp / masa_total_temp if masa_total_temp > 0 else 0
    
    # Paso 2: Verificar si necesita ajuste de acidez
    if acidez_resultante <= acidez_limite_critica:
        # No necesita ajuste
        return {
            "status": "Optimal",
            "message": "Solución encontrada sin necesidad de Sustrato S",
            "ajuste_sustrato_s": None,
            "solucion_temporal": {
                "masa_total": masa_total_temp,
                "acidez_resultante": acidez_resultante,
                "crioscopia_resultante": crioscopia_resultante,
            }
        }
    
    # Paso 3: Calcular ajuste con Sustrato S
    ajuste = calcular_ajuste_sustrato_s(
        acidez_actual=acidez_resultante,
        acidez_objetivo_max=min(acidez_limite_critica, a_max),
        volumen_leche=volumen_obj_L,
        crioscopia_inicial=crioscopia_resultante,
    )
    
    if not ajuste["valido"]:
        return {
            "status": "Infeasible",
            "error": f"El ajuste con Sustrato S no es viable: {', '.join(ajuste.get('errores', []))}",
            "ajuste_sustrato_s": ajuste,
        }
    
    # Paso 4: Construir solución final con Sustrato S
    resultados = []
    for i in df.index:
        kg_usados = x_kg_temp[i].value()
        if kg_usados and kg_usados > 0.05:
            ...
            resultados.append({
                ...
                "Costo Parcial": round(kg_usados * df.loc[i, "Costo_Por_Kg"], 2),
            })
    
    # Agregar Sustrato S
    if ajuste["volumen_sustrato_s"] > 0:
        resultados.append({
            "Tanque": "Sustrato S (Ajuste Acidez)",
            "Litros a Bombear": round(ajuste["volumen_sustrato_s"], 2),
            "Masa (kg)": round(ajuste["volumen_sustrato_s"] * 1.0, 2),  # Densidad ~1.0
            "Grasa Origen": "0.00%",
            "Costo Parcial": 0.0,
            "Tipo": "Ajuste Químico",
        })
    ...
    return {
        "status": "Optimal",
        "message": "Solución encontrada con ajuste automático de Sustrato S",
        "resultados": resultados,
        "metricas": {
            "volumen_final": volumen_obj_L,
            "masa_total": masa_total_temp,
            "grasa_final": (grasa_total_temp / masa_total_temp) * 100 if masa_total_temp > 0 else 0,
            "crioscopia_final": ajuste["crioscopia_final"],
            "acidez_inicial": ajuste["acidez_inicial"],
            "acidez_final": ajuste["acidez_final"],
            "volumen_sustrato_s": ajuste["volumen_sustrato_s"],
        },
        "ajuste_sustrato_s": ajuste,
    }
